...
| `model`                | string           | `gemini-1.5-pro` | Gemini model name                                           |                                      |
...
* Optional “reviewer mode” that only proposes plans (no apply)
* Org-scale GitHub App installer (optional)
* Artifact uploads and richer run reports

---

## 17. Default templates (bootstrap content)

### 17.1 Default `POLICY.md`

Includes: deny workflow edits, deny secrets, small diffs, tests required, budgets.

### 17.2 Default `ROADMAP.md`

Includes: Current Objective + Now/Next/Later.

### 17.3 Default `.evolver/config.yml`

Includes: PR mode, budgets, deny paths, secret scan enabled, inferred commands when possible.

---

## 18. Acceptance criteria (definition of “done” for v1)

* Target repo installs with **only a workflow file**.
* Empty repo becomes a runnable starter project with tests and docs within 1–3 runs.
* Non-empty repo evolves incrementally and opens PRs with:

  * passing checks
  * clear changelog entry
  * roadmap alignment
* Policy violations prevent commits/PRs.
* No workflow edits unless explicitly enabled.
